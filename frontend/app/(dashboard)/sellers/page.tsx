  ...
  return (
    <section className="space-y-8 pb-10">
      <div className="max-w-3xl space-y-2">
        <p className="text-xs font-medium uppercase tracking-wide text-[var(--foreground-muted)]">Seller fingerprinting</p>
        <h2 className="text-3xl font-semibold tracking-tight text-[var(--foreground)]">Cross-marketplace seller clusters.</h2>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="flex items-center gap-3.5">
          <div className="flex h-10 w-10 items-center justify-center rounded-[var(--radius-lg)] bg-[var(--accent-soft)]">
            <Fingerprint className="h-5 w-5 text-[var(--accent)]" strokeWidth={1.8} />
          </div>
          <div>
            <p className="text-xl font-semibold text-[var(--foreground)]">{clusters.length}</p>
            <p className="text-xs font-medium text-[var(--foreground-muted)]">Clusters</p>
          </div>
        </Card>
        <Card className="flex items-center gap-3.5">
          <div className="flex h-10 w-10 items-center justify-center rounded-[var(--radius-lg)] bg-[var(--accent-soft)]">
          ...
          <div>
            <p className="text-xl font-semibold text-[var(--foreground)]">{totalSellers}</p>
            <p className="text-xs font-medium text-[var(--foreground-muted)]">Tracked sellers</p>
          </div>
        </Card>
        <Card className="flex items-center gap-3.5">
          <div className="flex h-10 w-10 items-center justify-center rounded-[var(--radius-lg)] bg-[var(--status-error-bg)]">
            <ShieldAlert className="h-5 w-5 text-[var(--status-error-text)]" strokeWidth={1.8} />
          </div>
          <div>
            <p className="text-xl font-semibold text-[var(--foreground)]">{highRiskCount}</p>
            <p className="text-xs font-medium text-[var(--foreground-muted)]">High-risk clusters</p>
          </div>
        </Card>
      </div>
      ...
      <Card>
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <h3 className="mt-0.5 text-lg font-semibold tracking-tight text-[var(--foreground)]">Clusters</h3>
          </div>
          ...
          {loading ? (
            ...
            clusters.map((cluster) => (
              <div
                ...
                className="rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--panel-muted)] p-4"
              >
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-semibold text-[var(--foreground)]">{cluster.open_violation_count} open violations</p>
                ...
                <div className="mt-3 space-y-1.5 border-t border-[var(--border)] pt-3">
                  {cluster.sellers.map((seller) => (
                    <div key={seller.seller_id} className="flex items-center justify-between gap-3 rounded-[var(--radius-sm)] bg-[var(--panel)] px-3 py-2">
                      <div className="min-w-0">
                        {seller.storefront_url ? (
                          ...
                          <p className="text-sm font-semibold text-[var(--foreground)]">{seller.seller_name}</p>
                        )}
